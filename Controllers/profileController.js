...
exports.saveProfilePicture = async (req, res) => {
    ...
    try {
      ...
      if (user.profilePicture && fs.existsSync(path.join(__dirname, '..', 'public', 'uploads', user.profilePicture))) {
        fs.unlinkSync(path.join(__dirname, '..', 'public', 'uploads', user.profilePicture));
      }
  
      user.profilePicture = req.file.filename;  // Save new image filename in the database
      await user.save();
